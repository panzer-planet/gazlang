...
| Files and programs | `read_file`, `write_file`, `file_exists`, `list_dir`, `make_dir`, `delete_file`, `read_stdin`, `args`, `getenv`, and `run(["git", "status"])`, which starts a program with no shell in between |

...
class BytecodeTest extends GazLangTestCase
{
    ...
    {
        $this->expectExceptionMessage('Bytecode version 4, but this is GazLang bytecode 3');
        self::succeed([], "GAZLANG BYTECODE 4\nglobals\n\ntop\nlocals\n");
    }
    ...
    private function programs(): array
    {
        ...
        try {
            $files = [...glob('tests/programs/*.gaz'), ...glob('lib/*.gaz'), ...glob('tests/gaz/*/*.gaz'), ...glob('compiler/*.gaz'), ...glob('vm/bench/gaz/*.gaz')];
        } finally {
            chdir($cwd);
        }
